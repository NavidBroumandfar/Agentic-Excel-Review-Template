# ⚠️ Compliance Notice:
# Assistive mode only. Do NOT overwrite validated cells/ranges or macros in the source workbook.
...
# ---- Config defaults
DEFAULT_WORKBOOK = "data/Sample_Review_Workbook.xlsx"
DEFAULT_SHEET_IN = "ReviewSheet"
DEFAULT_SHEET_OUT_PREFIX = "AI_ReviewSheet"
LOG_PATH = "logs/excel_writer.jsonl"
...
def load_review_sheet(
    ...
    if from_m1:
        ...
        try:
            from src.excel.excel_reader import read_review_sheet  # M1 entrypoint
            from src.utils.config_loader import load_config
        except Exception as e:
            raise RuntimeError("Failed to import/read via M1 (excel_reader).") from e

        # Load config and read data
        config = load_config()
        df, _ = read_review_sheet(config)
        return df
    else:
        if not input_csv:
...
def _next_available_sheet_name(existing: List[str], base: str) -> str:
    # base should already include date: e.g., AI_ReviewSheet_20251028
...
def run_writer(
    ...
    # 2) load input
    df = load_review_sheet(from_m1=from_m1, sheet_in=sheet_in, input_csv=input_csv)
...
def main(argv=None):
    p = argparse.ArgumentParser(
        description="Safe Excel writer for AI review outputs (M3)."
    )
