...
def create_draft(
    ...
    record = {
        ...
        "format": fmt,
        "attachments": [
            str(a.relative_to(ROOT)) if str(a).startswith(str(ROOT)) else str(a)
            for a in att_files
        ],
        "reviewer": reviewer,
        "timestamp": ts_iso,
...
def main():
    ap = argparse.ArgumentParser(
        description="M9.1 — Create Outlook draft (.msg on Windows, .eml elsewhere)"
    )
    ap.add_argument("--month", required=True, help="YYYYMM or YYYY-MM")
    ap.add_argument(
        "--locale", choices=["en", "fr"], default="en", help="Email language"
    )
    ap.add_argument("--subject", required=True, help="Subject line")
    ap.add_argument(
        "--html",
        help="Explicit HTML path; otherwise uses M9 output in /outputs/publication/",
    )
    ...
    ap.add_argument(
        ...
        nargs="*",
        default=[],
        help="Attachment paths (relative to repo root or absolute)",
    )
    ap.add_argument("--reviewer", required=True, help="Your name for QA log")
    ap.add_argument(
        "--force-eml", action="store_true", help="Force .eml even on Windows"
    )
    args = ap.parse_args()
